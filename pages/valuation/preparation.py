...
from pages.valuation.preparation_display import (
    render_charts, render_standardized, render_ratios, render_averages,
)
from pages.valuation.preparation_recommendations import render_recommendations
...
def render_preparation(ticker: str, company) -> None:
    ...
    div_data = _get_dividend_data(data["ticker"])
    recs = generate_recommendations(data, div_data)
...
def _render_single_flag(f: dict, css: str) -> None:
    cat = _CAT_LABELS.get(f.get("category", ""), "Other")
    yr, what = f.get("year", ""), f.get("what", "")
    imp = f.get("impact_mn")
    imp_s = (f" | ${imp/1000:.1f}B" if imp and abs(imp) >= 1000
             else f" | ${imp:.0f}M" if imp else "")
    causes = f.get("possible_causes")
    causes_s = ('<br><span style="opacity:0.6;font-size:12px">'
                f"Possible: {' / '.join(causes[:3])}</span>" if causes else "")
    st.markdown(f'<div class="{css}"><strong>{yr}</strong> '
        f'<span class="source-tag">{cat}</span>{imp_s}<br>'
        f'{what}{causes_s}</div>', unsafe_allow_html=True)


def _get_dividend_data(ticker: str) -> dict | None:
    """Fetch DDM data at preparation time, cache in session_state.

    Reuses the same session_state key as the DDM tab so data is
    fetched once and shared across modules.
    """
