"""Test Simple DCF on 100 DCF-suitable companies and analyze results."""
...
TICKERS = [
    ...
    "PANW", "QCOM", "IBM", "CDNS", "SNPS", "ANSS", "FTNT", "ANET", "WDAY",
...
def run_test(ticker):
    ...
    # CapEx, D&A, SBC: historical avg, capped
    if hist and len(hist) >= 2:
        capex_pct = min(avg_hist(hist, "capex_pct", 0.05), 0.20)
        da_pct = avg_hist(hist, "da_pct", 0.03)
        sbc_pct = min(avg_hist(hist, "sbc_pct", 0.01), 0.08)
    else:
        capex_pct = min(abs(cf.get("capex", 0) or 0) / revenue, 0.20)
        da_pct = abs(cf.get("depreciation", 0) or 0) / revenue if cf.get("depreciation") else 0.03
        sbc_pct = min(abs(cf.get("sbc", 0) or 0) / revenue, 0.08)
    ...
    result = run_dcf(
        fcf_table, wacc_r, 0.025, "gordon", None,
...
def analyze(results):
    ...
    print(f"\n{'=' * 70}")
    print(f"  DCF TEST — {len(results)} DCF-suitable companies")
    print(f"{'=' * 70}")
    ...
    print(f"\n  Accuracy (DCF vs Current):")
    for pct in [10, 20, 30, 50]:
        n = sum(1 for x in dcf_vs_cur if -pct / 100 <= x <= pct / 100)
        print(f"    Within +/-{pct}%: {n}/{len(results)} ({n / len(results):.0%})")
    ...
    for d in by_anl[:15]:
        print(f"    {d['ticker']:<6} DCF:${d['dcf']:>8.2f}  Anl:${d['analyst']:>8.2f}  "
              f"Diff:{d['dcf_vs_anl']:+.1%}  Cur:${d['current']:.2f}")
...
if __name__ == "__main__":
    ...
    with open("data/dcf_test_suitable.json", "w") as f:
